...
如果云端 noVNC 被 Vercel Security Checkpoint 拦截，可在本机真实浏览器登录 `https://www.anything.com`，用 Cookie-Editor 一类工具导出 `anything.com` Cookie JSON，然后在控制台点「导入 Cookie」。Cookie 里包含 `refresh_token`、Auth.js session 或 `lS_authToken` 之一即可尝试导入；系统会用 Cookie 直连 GraphQL，不再依赖云端浏览器打开登录页。
