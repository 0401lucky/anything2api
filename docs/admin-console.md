...
- **导入 Cookie**：云端 noVNC 被 Vercel 拦截时，在本机真实浏览器登录 anything.com，导出 Cookie JSON 后粘贴到控制台。建议同时包含 `lS_authToken` 和 `refresh_token`；只有 `refresh_token` 时可以粘贴 `refresh_token=...` 或裸 JWT，系统会先请求页面收集服务端补发的 Cookie，再尝试直连 GraphQL。
